...
# ---- CYBER ----
sub(C, 'A self-propagating credential-stealing worm poisoned 1,684 versions of 420 npm packages in the keyv/cacheable family in roughly half an hour — planting Claude Code and VS Code execution hooks along the way — while',
      'A self-propagating credential-stealing worm has now poisoned 2,234 versions of 444 npm packages across twelve organizations — SafeDep’s updated registry-backed count, up from 1,684 earlier — planting Claude Code and VS Code execution hooks along the way, while')
