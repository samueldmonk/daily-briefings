#!/usr/bin/env python3
"""Programmatic validation for the 5:06 PM ET Afternoon Edition, Aug 26 2026."""
...
ck('DIRECTION ONLY' in TWS and 'both rejected in The Lead' in TWS,
   'NVDA magnitude explicitly withheld')
# 5:06 -- BOTH offered NVDA after-hours percentages must appear ONLY inside rejection prose.
for bad in (r'1\.45%', r'\$213\.70', r'\$210\.65'):
    for hit in re.finditer(bad, TWS):
        w = TWS[max(0, hit.start() - 500):hit.start() + 300].lower()
        ck('neither is published' in w or 'reject' in w or 'not published' in w,
           'rejected NVDA quote-page figure %s outside rejection context' % bad)
for hit in re.finditer(r'1\.59%', TWS):
    w = TWS[max(0, hit.start() - 600):hit.start() + 400].lower()
    ck('closed down' in w or 'regular-session close' in w or 'neither is published' in w
       or 'on the session' in w or 'into the print red' in w,
       '-1.59% must be labelled the REGULAR-SESSION close, never the after-hours move')
ck('is NVDA&rsquo;s regular-session close' in WS, 'the -1.59% relabel is explicitly called out')
...
for tic in ('Williams-Sonoma', 'WSM', 'Abercrombie'):
    ck(not re.search(re.escape(tic) + r'[^.]{0,80}(?:rose|fell|jumped|soared|sank)\s+\d+%', TWS),
       'no invented move for %s' % tic)
ck('No results and no after-hours prices for either appeared' in TWS, 'unreported names declared unknown')

# ---------- 4b. 5:06 SOURCED AFTER-HOURS MOVES (three winners, one direction-only) ----------
for s_ in ('about 15% in extended trading', '$1.05', '97 cents', '$805', '$795', '$728', '$116', '65 cents', '$67', '37 cents', 'Todd McKinnon'):
    ck(s_ in TWS, 'WS carries Okta figure %s' % s_)
ck(abs(805/728 - 1 - 0.1057) < 0.001, 'Okta +11% revenue growth re-derived (10.58%)')
ck(round(116/67 - 1, 2) == 0.73, 'Okta net income +73% re-derived')
ck(abs(0.65/0.37 - 1 - 0.7568) < 0.001, 'Okta per-share +75.7% re-derived')
for s_ in ('as much as 12% in extended trading', '$5.99', '$6.01', '$5.94', '$1.53', '$1.52', 'ninth consecutive'):
    ck(s_ in TWS, 'WS carries CrowdStrike 5:06 figure %s' % s_)
ck(5.99 > 5.94 and 1.53 > 1.52, 'CRWD guides above consensus on both lines')
for s_ in ('$89.02', '116.6%', '18.3%', 'Blackwell Ultra'):
    ck(s_ in TWS, 'WS carries NVDA Data Center 5:06 figure %s' % s_)
ck(abs(89.02/2.166 - 41.10) < 0.05, 'NVDA DC +116.6% implies a 41.10B year-ago base')
ck(abs(89.02/1.183 - 75.25) < 0.05, 'NVDA DC +18.3% implies a 75.25B prior quarter')
ck('rise 50% year over year' in TWS and 'since 2021' in TWS, 'FactSet 50% earnings-growth frame carried')
ck('Okta' in TIX and 'CrowdStrike' in TIX, 'index Markets card names the night\'s winners')
ck('fourteenth consecutive edition' in TCY, 'cyber KEV-static count advanced to fourteenth')
ck('Adobe and NVIDIA each published several advisories' in TCY and 'no CVE, no CVSS' in TCY,
   'Adobe/NVIDIA advisories printed as seen-not-published, with no invented CVE')
ck('thirtieth consecutive edition' in TMM, 'MMA champions board unchanged count advanced')
ck('Golden 1 Center' in TMM and 'Oriental Sports Center' in TMM, 'MMA both calendar anchors re-verified')
...
for k, v in F.items():
    for m in re.finditer(r'class="tag new">([^<]*)<', v):
        ck('5:06' in m.group(1), '%s: stale "New" tag -> %s' % (k, m.group(1)))
...
for k, v in F.items():
    for m in re.finditer(r'New\s*&middot;\s*(\d{1,2}:\d{2})', v):
        ck(m.group(1) == '5:06', '%s: prose still labelled "New &middot; %s"' % (k, m.group(1)))
ck(WS.count('class="tag new"') + WS.count('New &middot; 5:06') >= 1, 'WS has at least one genuinely new item')
ck(CY.count('class="tag new"') + CY.count('New &middot; 5:06') >= 1, 'CY has at least one genuinely new item')
ck(MM.count('class="tag new"') + MM.count('New &middot; 5:06') >= 1, 'MM has at least one genuinely new item')
