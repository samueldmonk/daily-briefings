#!/usr/bin/env python3
"""Fact-check gate for the 5:35 PM edition. Guards are NARROWED, never loosened."""
import re, sys, io, os
REPO = sys.argv[1]
...
for n, h in PAGES.items():
    ...
    ck('id="updated">5:35 PM ET' in h, "%s: stamp not 5:35 PM" % n)
    ck("Data as of 5:35 PM ET" in h, "%s: freshness fallback not 5:35 PM" % n)
...
for n, h in PAGES.items():
    stale = re.findall(r'New &middot; (?!5:35 PM)([0-9]{1,2}:[0-9]{2} [AP]M)', h)
...
for cve in ("CVE-2026-82078", "CVE-2026-81578", "CVE-2026-3055"):
    ck(not near(CY, cve, "9.8", 260), "cyber: 9.8 attached to %s" % cve)
...
# ---------- 15. SCRUB ARTEFACTS ----------------------------------------------
for n, h in PAGES.items():
    for bad in ("an earlier edition edition", "at an earlier edition", "Carried &middot; Aug 31, 5:35 PM",
                "&middot; &middot;", "<b><b><b>"):
...
print("validate_1735: %d checks, %d failures" % (checks, len(fails)))
for f in fails:
    print("  FAIL:", f)
