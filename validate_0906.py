#!/usr/bin/env python3
"""Validator for the 2026-08-27 ~9:06am ET Morning Edition.
...
if '232,000' in WS:
    ...
    checks += 1
# as-of time stated
ck('<b>8:40 AM ET</b>' in WS and 'New \u00b7 9:05 AM ET' in WS, 'ws: as-of time missing')
...
# ── additional checks for the 9:06 AM ET edition (third run of 2026-08-27) ────
CY = D['cyber-briefing.html']; MM = D['mma-briefing.html']; IX = D['index.html']

# Markets: verified 203,000 claims print; the rejected 232,000 kept as rejection only
ck('203,000' in WS, 'ws: verified jobless-claims figure missing')
ck('week ending <b>August 22</b>' in WS, 'ws: claims reference week missing')
...
for _m in re.finditer(r'Jackson Hole', WS):
    _w = WS[max(0, _m.start()-400):_m.start()+300]
    ck('removed' in _w.lower(), 'ws: Jackson Hole framing not scoped to its removal note')
for _m in re.finditer(r'\$5\.90', WS):
    _w = WS[max(0, _m.start()-400):_m.start()+400]
    ck('not published' in _w.lower(), 'ws: $5.90 EPS appears outside a rejection context')
ck('$1.47 billion' in WS and '$0.31' in WS, 'ws: CrowdStrike quarter figures missing')
ck('8.9%' in WS, 'ws: CrowdStrike pre-open move missing')
ck('nearly 12%' in WS, 'ws: Salesforce pre-open move missing')
ck('Nasdaq 100 futures up 0.9%' in WS, 'ws: Thursday futures read missing')
ck('QQQ +1.04%' in WS, 'ws: ETF proxy read missing')
ck('After-Hours Movers' not in WS, 'ws: After-Hours section present pre-open')
ck('4.64' in WS and '4.66' in WS, 'ws: 10-year yield range missing')

# Cyber: CareCloud named; anonymous-figure note; Gunra; FBI water; directive framing
ck('CareCloud' in CY, 'cyber: CareCloud item missing')
ck('3.7 million' in CY and '350,000' in CY, 'cyber: CareCloud scope figures missing')
ck('March 10 and March 16' in CY, 'cyber: CareCloud intrusion window missing')
...
ck('Hospital for Sick Children' in CY, 'cyber: SickKids item missing')
ck('University of Texas at San Antonio' in CY, 'cyber: UTSA item missing')
...
ck(CY.count('August 27') >= 3, 'cyber: Oracle Aug 27 deadline not repeated in all three places')
ck('CVE-2026-8037' in CY and 'Progress LoadMaster' in CY, 'cyber: Aug 7 KEV addition missing')

# MMA: new UFC 331 detail; no champions-board change
ck('seven-fight winning streak' in MM, 'mma: Van streak missing')
ck('four-defence reign' in MM, 'mma: Pantoja reign detail missing')
...
ck('9 p.m. ET / 6 p.m. PT' in MM, 'mma: UFC 331 start time missing')
ck("China's highest-ranked male UFC contender" in MM, 'mma: Song context missing')

# index cards agree with the page leads
ck('203,000' in IX, 'index: markets card does not reflect the page lead')
ck('Nasdaq futures roughly 0.9%' in IX, 'index: markets card futures figure missing')
ck('CVE-2026-21962' in IX, 'index: cyber card lead missing')
