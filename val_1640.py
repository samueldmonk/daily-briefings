# -*- coding: utf-8 -*-
"""Validator for the 1640 edition. python3 val_1640.py <repodir>"""
...
prior_cy = io.open(os.path.join(R, 'archive', 'cyber-%s.html' % PRIOR),
                   encoding='utf-8').read()
for token in ['Raiu', 'busybox', '7.23.5']:
    ck(token not in prior_cy, 'cyber: New item "%s" already in prior snapshot' % token)
    ck(token in P['cyber-briefing.html'], 'cyber: New item "%s" missing from page' % token)
# the stale CPI New tag must be gone (it was new in the 1611 edition, not this one)
ck('Consensus is now sourced.</b>' not in w, 'ws: stale CPI New framing still present')
...
ck('UFC.com re-fetched this run and unchanged' in m, 'mma: refusal not re-tested this run')
...
# --- 13. Raiu material must be attributed to Security Affairs, not asserted first-hand
cy = P['cyber-briefing.html']
ck('Security Affairs' in cy, 'cyber: Raiu material lacks Security Affairs attribution')
ck('securityaffairs.com/198538' in cy, 'cyber: Security Affairs source URL missing')
ck('Medium post was not fetched this run' in cy, 'cyber: provenance caveat missing')
# the two RouterOS patch lists must be printed side by side, never merged
ck('printed side by side rather than merged' in cy, 'cyber: version-list refusal lost')
# MMA: the double-booking refusal must be stated
ck('cannot be booked twice on one night' in P['mma-briefing.html'],
   'mma: Belgaroui double-booking refusal missing')
# CPI: no single consensus asserted
ck('no single consensus figure is asserted' in w, 'ws: CPI conflict not printed')
